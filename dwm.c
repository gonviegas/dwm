...
struct Client {
  ...
  int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen,
      isfakefullscreen, issticky, ispermanent;
...
static void centerfloatingtoggle(const Arg *arg);
...
static void focusmonmoveclient(const Arg *arg);
static void focusmonmovetag(const Arg *arg);
...
static void shiftviewmoveclient(const Arg *arg);
static void shiftviewocc(const Arg *arg);
static void shiftviewoccmoveclient(const Arg *arg);
static void swaptag(const Arg *arg);
static void shiftviewswaptag(const Arg *arg);
static void shiftviewoccswaptag(const Arg *arg);
...
void buttonpress(XEvent *e) {
  ...
  if (ev->window == selmon->barwin) {
    ...
    if (i < LENGTH(tags)) {
      ...
      click = ClkLtSymbol;
    } else if (ev->x > (x = selmon->ww - (int)TEXTW(stext) + lrpad / 2 - stw)) {
      click = ClkStatusText;
      char *text = rawstext;
      int i = -1;
      char ch;
...
void centerfloatingtoggle(const Arg *arg) {
  if (selmon->sel->isfloating) {
    resize(selmon->sel, selmon->sel->x, selmon->sel->y, selmon->sel->w,
           selmon->sel->h, borderpx, 0);
    selmon->sel->x =
...
void drawbar(Monitor *m) {
  ...
  if (statusallmons == 1) {
    if (m == selmon || 1) { /* status is drawn on all monitors */
      ...
      tw = TEXTW(stext) - lrpad / 2; /* add 2px for extra right padding */
  ...
  for (c = m->clients; c; c = c->next) {
    ...
    if (tagicons) {
      for (i = 0; i < LENGTH(tags); i++) {
        if ((!z[i] || z[i]->isfloating) && c->tags & (1 << i) && !c->isfloating)
          z[i] = c;
        else if (!z[i] && c->tags & (1 << i))
          z[i] = c;
      }
    }
  }
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    if (hidevacanttags) {
      ...
      if (urg & 1 << i) {
        ...
        drw_setscheme(drw, scheme[SchemeTagsEmpt]);
      }
    }
    if (tagicons && z[i] != NULL && z[i]->icon) {
      drw_text(drw, x, 0, w, bh, lrpad / 2, " ", 0);
      drw_pic(drw, x + boxw + TAGICON_X, TAGICON_Y + (bh - z[i]->ich),
              z[i]->icw, z[i]->ich, z[i]->icon);
    } else {
  ...
  w = TEXTW(m->ltsymbol);
  drw_setscheme(drw, scheme[SchemeLayoutSymb]);
  x = drw_text(drw, x, 0, w, bh, lrpad / 2, m->ltsymbol, 0);

  if ((w = m->ww - tw - stw - x) > bh) {
    if (m->sel) {
      int mid = (m->ww - tw - stw + x - (int)TEXTW(m->sel->name)) / 2 - x;
      /* make sure name will not overlap on tags even when it is very long */
      mid = mid >= lrpad / 2 ? mid : lrpad / 2;
      drw_setscheme(drw, scheme[m == selmon ? SchemeInfoSel : SchemeInfoInv]);
      if (m->sel->isfloating || m->sel->issticky) {
        drw_text(drw, x, 0, w, bh,
                 mid + (m->sel->icon ? m->sel->icw + INFO_X : 0), m->sel->name,
                 0);
        if (m->sel->isfloating)
          drw_rect(drw, x + mid + INDIC_X, boxs, boxw, boxw, m->sel->isfixed,
                   0);
        if (m->sel->issticky)
          drw_rect(drw, x + mid + INDIC_X,
                   user_bh ? user_bh - 5 : boxs + drw->fonts->h - 6, boxw, boxw,
                   1, 0);
        if (m->sel->icon)
          drw_pic(drw, x + mid + ICON_X, (bh - m->sel->ich) / 2, m->sel->icw,
      ...
      drw_setscheme(drw, scheme[m == selmon ? SchemeInfoNorm : SchemeInfoInv]);
...
static void focusmonmoveclient(const Arg *arg) {
  ...
  if (!selmon->sel || !mons->next || dirtomon(arg->i)->pertag->curtag == 0)
    return;
  if ((m = dirtomon(arg->i)) == selmon)
    return;
...
void placemouse(const Arg *arg) {
  int x, y, px, py, ocx, ocy, nx = -9999, ny = -9999;
  ...
  if (!(c = selmon->sel) ||
    ...
    return;
  if (c->isfullscreen &&
      !c->isfakefullscreen) /* no support placing fullscreen windows by mouse */
    return;
  restack(selmon);
  prevr = c;
  if (XGrabPointer(dpy, root, False, MOUSEMASK, GrabModeAsync, GrabModeAsync,
...
void setmfact(const Arg *arg) {
  ...
  if (!arg || !selmon->lt[selmon->sellt]->arrange || selmon->sel->isfloating)
    return;
  f = arg->f < 1.0 ? arg->f + selmon->mfact : arg->f - 1.0;

  if (arg->f == 0.0)
    f = 0.50;
  else if (f < 0.10 || f > 0.90)
    return;
...
void shiftviewmoveclient(const Arg *arg) {
  if (selmon->pertag->curtag == 0) {
    return;
  }
...
void shiftviewocc(const Arg *arg) {
  Arg shifted;
  Client *c;
...
void shiftviewoccmoveclient(const Arg *arg) {
  if (selmon->pertag->curtag == 0) {
    return;
  }
...
// Helper Function for shiftviewmoveall and shiftviewoccmoveall
void swaptag(const Arg *arg) {
  Client *c;
...
void shiftviewswaptag(const Arg *arg) {
  if (selmon->pertag->curtag == 0) {
    return;
  }
  ...
  if (arg->i > 0) /* left circular shift */
    ...
    shifted.ui =
        (shifted.ui >> (-arg->i) | shifted.ui << (LENGTH(tags) + arg->i));
  // uncomment if you also want to "go" (view) the tag where the the clients are
  // going
  swaptag(&shifted);
  view(&shifted);
}

void shiftviewoccswaptag(const Arg *arg) {
  if (selmon->pertag->curtag == 0) {
    return;
  }
...
void togglefakefullscreen(const Arg *arg) {
  Client *c = selmon->sel;
  if (!c || (c->isfullscreen && c->isfakefullscreen == 0))
    return;

  if (c->isfakefullscreen != 1 &&
...
static void bstack(Monitor *m) {
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
    if (i < m->nmaster) {
      resize(c, mx, my,
             mw * (c->cfact / mfacts) + (i < mrest ? 1 : 0) - (2 * bw),
             mh - (2 * bw), bw, 0);
      mx += WIDTH(c) + iv;
    } else {
      resize(c, sx, sy,
...
void centeredmaster(Monitor *m) {
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
    if (!m->nmaster || i < m->nmaster) {
      ...
      resize(c, mx, my, mw - (2 * bw),
             mh * (c->cfact / mfacts) + (i < mrest ? 1 : 0) - (2 * bw), bw, 0);
      my += HEIGHT(c) + ih;
    } else {
      /* stack clients are stacked vertically */
      if ((i - m->nmaster) % 2) {
        resize(c, lx, ly, lw - (2 * bw),
               lh * (c->cfact / lfacts) +
                   ((i - 2 * m->nmaster) < 2 * lrest ? 1 : 0) - (2 * bw),
               bw, 0);
        ly += HEIGHT(c) + ih;
      } else {
        resize(c, rx, ry, rw - (2 * bw),
               rh * (c->cfact / rfacts) +
                   ((i - 2 * m->nmaster) < 2 * rrest ? 1 : 0) - (2 * bw),
               bw, 0);
        ry += HEIGHT(c) + ih;
      }
    }
...
void horizgrid(Monitor *m) {
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
    if (i < ntop) {
      ...
      resize(c, sx, sy,
             sw * (c->cfact / sfacts) + ((i - ntop) < srest ? 1 : 0) - (2 * bw),
             sh - (2 * bw), bw, 0);
      sx += WIDTH(c) + iv;
    }
}
